...
class TestComponent extends Component {
  ...
  handleTestUpdateProfile = async () => {
    ...
    let userDocRef = await firestore
      .collection('user')
      .doc('zkcwbY0qf5dtCPblF1p8hgdZRu63');
    try {
  ...
  handleTestJoinEvent = async () => {
    ...
    try {
      await eventDocRef.update({
        [`attendees.zkcwbY0qf5dtCPblF1p8hgdZRu63`]: attendee
      });
      toastr.success('Success');
    } catch (error) {
      console.log(error);
  ...
  handleTestCancelGoingToEvent = async () => {
    ...
    try {
      await eventDocRef.update({
        [`attendees.zkcwbY0qf5dtCPblF1p8hgdZRu63`]: firebase.firestore.FieldValue.delete()
      });
      toastr.success('Success');
    } catch (error) {
      console.log(error);
  ...
  handleTestChangeAttendeePhotoInEvent = async () => {
    ...
    try {
      await eventDocRef.update({
        [`attendees.zkcwbY0qf5dtCPblF1p8hgdZRu63.photoURL`]: 'testing123.jpg'
      });
      toastr.success('Success');
    } catch (error) {
      console.log(error);
    ...
    return (
      <div>
        ...
        <Button
          ...
          onClick={(e) => !loading && incrementAsync(e.target.name)}
          positive
          content='Increment'
        />
        <Button
          ...
          onClick={
            (e) => !loading && decrementAsync(e.target.name)
            //custom: '!loading' prevents button from being clicked until finished loading
          }
          negative
          content='Decrement'
        />
